...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("callToUser", (data) => {
      setReceiveCall(true);  // Set state to indicate an incoming call.
  ...
  const startCall = async () => {
    try {
      ...
      setCallerWaiting(true);//Waiting to join receiver
      ...
      peer.on("stream", (remoteStream) => {
        if (receiverVideo.current) {
          receiverVideo.current.srcObject = remoteStream; // ✅ Assign remote stream to video element
          receiverVideo.current.muted = false; // ✅ Ensure audio from the remote user is not muted
      ...
      socket.once("callAccepted", (data) => {
        ...
        setCallerWaiting(false);//receiver join the call
  ...
  const handleAcceptCall = async () => {
    ...
    try {
      ...
      // ✅ Update call state
      setCallAccepted(true); // ✅ Mark call as accepted
      setReceiveCall(true); // ✅ Indicate that the user has received the call
      setCallerWaiting(false);//receiver join the call
  ...
  const handleRejectCall = () => {
    ...
    setCallerWaiting(false);//receiver reject the call
    setReceiveCall(false); // ✅ The user is no longer receiving a call
  ...
  const endCallCleanup = () => {
    ...
    setReceiveCall(false); // ✅ Indicate no ongoing call
